...
function slideUp(el) {
  ...
  return new Promise(function (onComplete) {
    _gsap.TweenLite.set(el, { overflow: 'hidden' });
    _gsap.TweenLite.to(el, duration / 1000, { height: 0, display: 'none', onComplete: onComplete });
  });
}
...
function slideDown(el) {
  ...
  return new Promise(function (onComplete) {
    _gsap.TweenLite.set(el, { overflow: 'hidden', height: 'auto', display: 'block' });
    _gsap.TweenLite.from(el, duration / 1000, { height: 0, onComplete: onComplete });
  });
}
...
function animate(els, to) {
  ...
  return new Promise(function (onComplete) {
    ...
    new _gsap.TimelineLite({ onComplete: onComplete }).staggerTo(target, duration / 1000, to, stagger / 1000);
  });
}
...
function animateFrom(els, from, to) {
  ...
  return new Promise(function (onComplete) {
    var target = Number.isInteger(els.length) ? els : [els];
    new _gsap.TimelineLite({ onComplete: onComplete }).staggerFromTo(target, duration / 1000, from, to, stagger / 1000);
  });
}
